...
selected_agent = "NNSARSALambda"  # TabTBQN, NNSARSALambda
...
agents = {
    "TabTBQN": MCarTabularTreeBackupAgent(path, load_model),
    "NNSARSALambda": MCarNNSARSALambdaAgent(path_nn, load_model, test_mode)
}
...
class MountainCarEnvBuilder(EnvBuilder):
    ...
    def episode_done(self, player_prop: Any):
        ...
        if self._episodes % self._save_after_episodes == 0 and save_model:
